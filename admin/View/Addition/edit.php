...
<div class="p-4">
    <div class="mb-2"><a href="/admin/addition/list"><i class="fa-solid fa-left-long"></i> Назад</a></div>
    ...
    <form action="" method="post">
        ...
        <div class="mb-4">
            ...
            <input type="text" class="form-control" name="name" value="<?php echo $addition['name']; ?>">
        </div>
        <div class="mb-4">
            <label for="" class="mb-2">Описание</label>
            <textarea name="description" id="description" cols="30" rows="10"><?php echo $addition['description']; ?></textarea>
        </div>
        <div class="mb-4">
            <label for="" class="mb-2">Пользователи</label>
            <select name="users-access[]" id="users-access" class="form-select js-chosen" data-placeholder="Выберите пользователя" multiple>
                <?php foreach ($users as $user): ?>
                    <option value="<?php echo $user['id']; ?>"
                        <?php foreach ($addition['users_access'] as $selectedUser) if($selectedUser == $user['id']) echo 'selected';?>
                    >
                        <?php echo $user['email'] ."(" . $user['name'] .' ' .$user['surname'] . ")"; ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
