...
class PRRCompany extends Model
{
    ...
    public function __construct(array $data = [])
    {
        ...
        if (count($data) > 0) {
            $prrCompany = $this->database->first("prr_company", $data);

            if(! $prrCompany)
                return false;

            foreach ($prrCompany as $key => $value) {
    ...
    public function save(): bool
    {
        ...
        if ($this->id > 0) {
            $stmt = $this->database->update("prr_company", $newData, ["id" => $this->id]);
            return $stmt;
        }
        else{
            $stmt = $this->database->insert("prr_company", $newData);
