...
<div class="sub-title-doc" style="font-size: 1.5rem">
    Файлы перевозчик 
    <i class="bi bi-info-circle text-primary" data-bs-toggle="tooltip" data-bs-placement="top"
        data-bs-custom-class="custom-tooltip"
        data-bs-title='Документы можно подгрузить если перетащить файл на тип нужного документа '></i>
</div>
...
<div class="quest__table-list-item list-item d-flex position-relative">
    ...
    <?php $counter = 1; 
    if(isset($application->carrier['expenses'])):
    foreach ($application->carrier['expenses'] as $expenses): ?>
...
<div class="quest__table-list-item list-item d-flex">
    <div class="list-item__number"></div>
    <div class="w-100 justify-content-around">
        <label class="mb-2">Дата получения всех документов (оригиналы) для нас (обязательное) (Бухгалтерия)</label>
        <input type="text" id="date-picker-receipt-all-documents" readonly class="form-control"
            value="<?php if ($application->dateReceiptAllDocumentsCarrier)
                        echo $application->dateReceiptAllDocumentsCarrier; ?>"
            data-id-application="<?php echo $application->id; ?>">
    </div>
</div>

<script type="module">
    const input = document.querySelector('#date-picker-receipt-all-documents');
    const defaultDate = input.value ? new Date(input.value) : null;

    new AirDatepicker(input, {
        dateFormat: 'dd.MM.yyyy',
        selectedDates: defaultDate ? [defaultDate] : [],
        buttons: ['clear'], // Убираем кнопку "Применить", если не нужна
        onSelect({
            date,
            formattedDate,
            datepicker
        }) {
            // Устанавливаем выбранную дату в поле (если не делает это автоматически)
...
<div class="quest__table-list-item list-item d-flex position-relative d-none">
    <div class="list-item__number">3</div>
    <div class="list-item__type-document">
...
<div class="quest__table-list-item list-item d-flex position-relative">
    <div class="list-item__number">5.3</div>
    <div class="list-item__type-document">
...
<div class="quest__table-list-item list-item d-flex position-relative">
    <div class="list-item__number">5.4</div>
    <div class="list-item__type-document">
