...
<body data-id-application="<?php echo $prrApplicationData['id']; ?>">
    ...
    <div class="card-wrapper wrapper">
        <div class="card__header row align-items-center">
            <div class="card__header-title col-10 d-flex align-items-center">
                № <?php echo $prrApplicationData['application_number']; ?>
                ...
                <?php if($prrApplicationData['id_application'] > 0): ?>
                    (<a href="/application?id=<?php echo $prrApplicationData['id_application']; ?>" style="color: blue;font-weight: 600 ">Привязанная заявка</a>)
                <?php endif; ?>

            </div>
            <div class="row col-2 justify-content-end align-items-center">
                <div class="card__header-data col-5 p-0"><?php echo date('d.m.Y', strtotime($prrApplicationData['date'])); ?></div>
                <div class="settings col-2 p-0 row justify-content-end">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-theme">
                            <li><a href="/prr/edit?id=<?php echo $prrApplicationData['id'];?>" class="dropdown-item">Редактировать</a></li>
                            <li><div class="dropdown-item js-copy-application" data-id-app="<?php echo $prrApplicationData['id']; ?>">Копировать</div></li>
        ...
        <div class="card__content d-flex">
            <div class="card__content-main d-flex flex-column">
                        ...
                        <div class="">
                            ...
                            <div class="settings-menu" id="setting-menu">
                                ...
                                <div class="item-doc-fotmat">
                                    <span class="type-format-doc" data-link="agreement-application">Договор-заявка с клиентом</span>
                                ...
                                <div class="item-doc-fotmat">
                                    <span class="type-format-doc" data-link="get-text-description">Текстовое описание</span>
                                </div>
                                <span class="type-format-doc" onclick="addTask(<?php echo $prrApplicationData['id'];  ?>);">Информация о счете</span>
                ...
                <div class="card__content-main-harakter_gruza">
                    <p><?php echo $prrApplicationData['nature_cargo_client']; ?></p>
                </div>
                    ...
                    <div class="form__header-menu-list d-flex w-100">
                        ...
                        <a class="mr-6 form__header-menu-item  ">
                            <div class="menu-item-click js-link-tab-change main-header-js-link-tab-change"
                                 data-container=".card__content-tab" onclick="loadComment()" data-tab-app="#tab-3" data-prefix-tab=".main-header">
                                Комментарии
                            </div>
                        </a>
                ...
                <div class="card__content-tab active" id="tab-2">
                    <?php $controller->extract(['prrApplication' => $prrApplication]); $controller->view('PRR/prr-application-doc'); ?>
                </div>
                <div class="card__content-tab" id="tab-3">
                    <?php $controller->view('PRR/prr-application-comment');  ?>
                </div>
            </div>
...
<!--                        <div class="mb-4">-->
<!--                            <span class="side-tab-span">Место разгрузки</span>-->
<!--                            <span class="side-main-span">-->
<!--                                --><?php //echo $prrApplicationData['city_client'] . ' ' .$prrApplicationData['address_client']; ?>
<!--                            </span>-->
<!--                        </div>-->
                        <div class="mb-4">
                            <span class="side-tab-span">Клиент</span>
                            <span class="side-main-span"><?php echo $client->get()['name']; ?></span>
                        </div>
                        <div class="mb-4">
                            <span class="side-tab-span">ПРР</span>
...
<!--                        </div>-->
                        ...
                        <div class="mb-4">
                            ...
                            <span class="side-main-span"><?php echo $prrApplicationData['taxation_type_prr']; ?></span>
                        </div>
                    </div>

                    <?php $controller->view('PRR/prr-prrCompany-dop-info'); ?>
...
<!--                        </div>-->
                    ...
                    <?php $controller->view('PRR/prr-client-dop-info'); ?>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(".application-comment-document").change(function () {
        ...
        $.ajax({
            method: 'POST',
            url: '/prr/application/ajax/comment-document',
    ...
    function deleteFile(id){
        $.ajax({
            url: "/prr/application/ajax/deleteFile",
            method: "POST",
    ...
    $('.form-upload-file').submit(function (e) {
        ...
        formData.append('application_id', <?php echo $prrApplication->id()?>);
        formData.append('doc_id', doc_id);

        let this_ = $(this);

        $.ajax({
            url: '/prr/application/ajax/uploadFiles',
            method: 'POST',
            processData: false,
            contentType: false,
            data: formData,
...
<script>
    ...
    //        url: '/prr/ajax/' + link,
    //        method: "POST",
    //        data: {id: <?php //echo $prrApplicationData['id']; ?>//},
    //        success: function (response) {
    //            let data = JSON.parse(response);
    //
    //            if(data['status'])
    ...
    //});
    <?php
        $nameDocumentApplicationClient = $client->get()['name'];

        $textPlace = '';

        foreach ($prrPlaceList as $place){
            if($place['type_for']){
                $textPlace = $place['city'];
            }
        }

        $nameDocumentApplicationClient .= ' ' . $textPlace.'_' . date('dmY',strtotime($prrApplicationData['date']));

    ?>
    const nameDocumentApplicationCarrier = '';
    const nameDocumentApplicationClient = '<?php echo $nameDocumentApplicationClient; ?>';
...
<script>
    $('.js-link-tab-change').click(function () {
        ...
        console.log(tabId,container,prefix)
