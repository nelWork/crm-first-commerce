...
<main class="journal container-fluid p-4">
    ...
    <ul class="nav nav-pills justify-content-center w-100 mb-4">
        ...
        <li class="nav-item">
            <a class="nav-link js-tab-task js-tab-customer" href="#" data-num-tab="2">
                Кредит
            </a>
        </li>
    </ul>
    ...
    <div id="tab-2" class="table-tab">
        <div class="table-container">
            ...
            <?php foreach($listPayment['arrayTo'] as $payment):
                $applicationsList = $payment['applicationDataList'];
                if(empty($applicationsList)) continue;
            foreach ($applicationsList as $key => $application ):
                ...
                >
                    ...
                    <td class="table-col-15 col-client
                        ...
                        <input type="number" min="0" id="actual-payment-client-<?php echo $application['id']; ?>" data-name-span="span-actual-payment-client"
                               data-id-application="<?php echo $application['id'] ?>" data-name-info="actual_payment_Client" data-date="<?php echo $payment['Дата']; ?>"
                        ...
                        <?php if($application['MARKER'] AND ! $application['fullPaymentClient']): ?>
                            <div style="font-weight: 800">
                                <span class="text-danger">*</span>
                                <?php echo $application['MARKER-comment']; ?>
                            </div>
                    ...
                    <?php if($key == 0): ?>
                        <td rowspan="<?php echo $count; ?>" style="vertical-align: middle; max-width: 250px">
                            ...
                            <?php
                                $textPayment = $payment['НазначениеПлатежа'];
                                foreach ($payment['Счета'] as $item){
                                    $textPayment = str_replace(
                                        $item,
                                        '<span class="number-app">' . $item .'</span>',
                                        $textPayment
                                    );
                                }
    ...
    <div class="modal-dialog modal-fullscreen" style="max-width: 100%">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="file" accept=".txt" id="file-dropzone" style="display: none;">
...
<script>
    $(document).ready(function() {
        ...
        $('#file-dropzone').change(function (event){

            uploadFile(document.getElementById('file-dropzone').files[0]);
        })
...
<script>
    ...
    $('body').on('change','.textarea-change-application-info',function () {
        ...
        $.ajax({
            ...
            data: {id: id, side: side, nameInfo: nameInfo, info: info, date: date, typeApp: typeApp},
            success: function (response) {
                console.log(response)
