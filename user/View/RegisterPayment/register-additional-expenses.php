    ...
    <main class="analytics">
        ...
        <section class="analytics-applications__list mb-5">
            <div class="container-fluid p-2 mb-5" style="background-color: #f5f5f5">
                <h1 class="text-center mb-5">Реестр доп. затрат</h1>
                ...
                <style>
                    ...
                    tr.selected {
                        ...
                        /* Цвет заливки строки по умолчанию */
                    }
                    ...
                    .filter-menu {
                        ...
                        max-width: 300px;
                        /* Максимальная ширина для предотвращения слишком больших меню */
                        width: auto;
                ...
                <style>
                    ...
                    table {
                        ...
                        /* Это объединяет соседние границы */
                        width: 100%;
                    }
                    ...
                    td {
                        border: 1px solid #dee2e6;
                        /* Устанавливаем границу для всех ячеек */
                        padding: 8px;
                ...
                <style>
                    ...
                    textarea.form-control:focus {
                        ...
                        /* Делаем фон прозрачным при фокусе */
                        border: none;
                        /* Убираем границу */
                        box-shadow: none;
                        /* Убираем тени */
                    ...
                    textarea.form-control {
                        ...
                        /* Убираем отступы */
                        font-family: inherit;
                        /* Шрифт как у остальных элементов */
                        font-size: inherit;
                        /* Размер шрифта такой же, как у остальных элементов */
                    }
                ...
                <style>
                    ...
                    .filter-icon.active {
                        ...
                        /* Зеленый цвет для активного фильтра */
                    }
                </style>
                <style>
                    .filtered-column {
                    ...
                    <table id="appTable" class="display">
                        <thead>
                            <tr>
                                ...
                                <th>Юр. лицо (комментарий)<i class="fas fa-filter filter-icon" data-column="2"></i></th>
                                <th>Вид доп. расхода <i class="fas fa-filter filter-icon" data-column="3"></i></th>
                                <th>Вид налогообложения <i class="fas fa-filter filter-icon" data-column="4"></i></th>
                                <th>Сумма <i class="fas fa-filter filter-icon" data-column="5"></i></th>
                                <th>Фактическая оплата, <br> клиента <i class="fas fa-filter filter-icon" data-column="6"></i></th>
                            </tr>
                        ...
                        <tbody>
                            <?php foreach ($listExpenses as $item):  ?>
                                <tr class="tr-application" 
                                data-id-app="<?php echo $item['id_application']; ?>" 
                                data-type-payment="<?php echo $item['type_payment_Client'] ;?>"
                                >
                                    <td><?php echo $item['user']; ?></td>
                                    <td class="link-application" data-id="<?php echo $item['id_application']; ?>"
                                        data-type="1">
                                    ...
                                    <td><?php echo $item['type_expenses']; ?></td>
                                    <td><?php echo $item['type_payment']; ?></td>
                                    <td class="td-remainder">
                                        <?php echo number_format($item['sum'],0,'',' '); ?> ₽
                                    </td>
                                    <td>
                                        <?php echo number_format($item['actual_payment_Client'],0,'',' '); ?> ₽ <br>
                                        <span class="text-secondary">
                                            (<?php echo number_format($item['transportation_cost_Client'],0,'',' '); ?> ₽)
                                        </span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const table = $('#appTable').DataTable({
                ...
                "columnDefs": [{
                        "targets": [0, 1, 2, 3, 4, 5, 6], // Здесь указываем номера столбцов, по которым не нужна сортировка
            ...
            $('.filter-icon').on('click', function(e) {
                ...
                // Проверяем, если фильтр по "Фактическая оплата, клиента" (data-column="6")
if ($(this).data('column') == 6) {
    const menu = $('#filterMenu');
    const optionsContainer = $('#filterOptions');
    const searchInput = $('#filterSearch');
    currentColumn = 6; // сохраняем текущий столбец
    optionsContainer.empty();

    // Наши варианты фильтра
    const paymentOptions = [
        {value: '1', label: 'Не оплачено'},
        {value: '2', label: 'Частично оплачено'},
        {value: '3', label: 'Полностью оплачено'},
    ];

    // Получаем текущее значение фильтра (если уже применён)
    const currentFilter = $('#filterMenu').data('currentPaymentFilter') || [];

    // Отрисовываем чекбоксы
    paymentOptions.forEach(opt => {
        const isChecked = currentFilter.includes(opt.value) ? 'checked' : '';
        optionsContainer.append(`
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="${opt.value}" id="payment-${opt.value}" ${isChecked}>
                <label class="form-check-label" for="payment-${opt.value}">${opt.label}</label>
            </div>
        `);
    });

    // Показываем фильтр в нужном месте
    ...
    optionsContainer.off('change').on('change', 'input[type="checkbox"]', function() {
        const selected = [];
        ...
        $('#filterMenu').data('currentPaymentFilter', selected);

        // Применяем фильтр по data-type-payment
        $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(fn => !fn._isPaymentFilter);
        $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
            const rowPaymentType = $($('#appTable').DataTable().row(dataIndex).node()).data('type-payment');
            if (selected.length === 0) return true;
            return selected.includes(String(rowPaymentType));
        });
        $.fn.dataTable.ext.search[$.fn.dataTable.ext.search.length - 1]._isPaymentFilter = true;

        table.draw();

        // Меняем цвет иконки при активном фильтре
        const icon = $('.filter-icon[data-column="6"]');
    ...
    return; // чтобы не выполнялся стандартный фильтр
}
        ...
        `);
            ...
            $('#clearFilter').on('click', function() {
                ...
                // updateFilterOptions();
                $('#filterMenu').removeData('currentPaymentFilter'); // очищаем память
$.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(fn => !fn._isPaymentFilter);
table.draw();
